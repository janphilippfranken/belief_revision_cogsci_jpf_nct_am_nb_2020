...
summary(mixed_means)
leveneTest(residuals(mixed_means)~m_dat_restr$cond_string)
hist(residuals(mixed_means))
co_means <- PrepareCoeffDf(data.frame(summary(mixed_means)$coefficients))
...
stdCoeffs <- modelOfInterest$Estimate
Predictors <- c("namee of predictor 1", "name of predictor 2")
coeffErr <- modelOfInterest$Std..Error
coeffDat <- data.frame(stdCoeffs, Predictors, coeffErr)
...
plotName <- coeffDat%>%
  ggplot(aes(x = Predictors, y = stdCoeffs, color = Predictors)) + 
  geom_point(size = 3, aes(shape = Predictors)) + theme(legend.position = "none") +
  geom_errorbar(aes(ymin=stdCoeffs-coeffErr, ymax=stdCoeffs+coeffErr), width=.2) + 
  geom_hline(yintercept=0, linetype = 'dashed', colour = 'grey', size = 0.5) + 
  theme_classic() +  
  My_Theme + 
  labs(title = "name of dependent variable", x="", y="") +
...
fullFigure <- ggarrange(namePlot1, namePlot2, namePlot3, namePlot4 + rremove("x.text"), 
          labels = c("A", "B", "C","D"),
          ncol = 2, nrow = 2)
